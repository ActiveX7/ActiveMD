...
const CREATOR_NUMBER = '255625316099';
...
const REGISTRY_SYNC_URL      = 'https://raw.githubusercontent.com/ActiveX7/ActiveMD/main/extPluginRegistry.json';
...
async function runAntilink(trashcore, m) {
  try {
    ...
    if (antilink && body.includes('http')) {
      await active.sendMessage(chatId, {
        delete: { remoteJid: chatId, fromMe: false, id: m.key.id, participant: m.key.participant }
      });
      active.sendMessage(chatId, {
...
function runAutoPresence(Active, m) {
  try {
    const chatId     = m.key.remoteJid;
